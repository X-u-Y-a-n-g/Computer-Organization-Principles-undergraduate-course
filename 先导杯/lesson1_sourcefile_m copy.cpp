...
void matmul_mpi(int N, int M, int P) {
    ...
    // 在根进程初始化数据
    std::vector<double> A;  // 只在根进程分配
    std::vector<double> C;  // 用于存储最终结果
    std::vector<double> C_ref;  // 用于验证
    if (rank == 0) {
        A.resize(N * M);
        init_matrix(A, N, M);
        init_matrix(B, M, P);
    ...
    // 使用MPI_Scatterv分发A的数据
    std::vector<int> sendcounts(size);
    std::vector<int> displs(size);
    if (rank == 0) {
        for (int i = 0; i < size; ++i) {
            sendcounts[i] = ((i < rows_remainder) ? rows_per_proc + 1 : rows_per_proc) * M;
            displs[i] = (i * rows_per_proc + std::min(i, rows_remainder)) * M;
        }
    }
    
    MPI_Scatterv(rank == 0 ? A.data() : nullptr, 
                 sendcounts.data(), displs.data(), MPI_DOUBLE,
                 local_A.data(), my_rows * M, MPI_DOUBLE, 
                 0, MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        for (int i = 0; i < size; ++i) {
            sendcounts[i] = ((i < rows_remainder) ? rows_per_proc + 1 : rows_per_proc) * P;
            displs[i] = (i * rows_per_proc + std::min(i, rows_remainder)) * P;
        }
    }
    ...
    if (rank == 0) {
        ...
        bool is_valid = validate(C, C_ref, N, P);
        std::cout << "[MPI] Valid: " << (is_valid ? "1" : "0") << std::endl;
        ...
        if (!is_valid) {
            ...
            std::cout << "[MPI] Max difference: " << max_diff 
...
int main(int argc, char** argv) {
    ...
    if (mode == "mpi") {
        ...
        if (rank == 0) {
            ...
            std::cout << "[MPI] Time: " << end_time - start_time << " seconds" << std::endl;
            std::cout << "[MPI] Memory: " << mem_end - mem_start << " MB" << std::endl;
        }
